...
if (isset($_POST['trigger_edit'])) {
    ...
    $edit_res = $conn->query("SELECT * FROM electrical_stock WHERE id = $id");
    ...
    if ($price <= 0 || $qty <= 0) {
        ...
        if (!empty($_POST['edit_id'])) {
            ...
            $sql = "UPDATE electrical_stock SET 
                    electrical_item_id='$item_id', total_qty='$qty', available_qty='$qty', 
            ...
            $sql = "INSERT INTO electrical_stock (electrical_item_id, total_qty, available_qty, bill_no, bill_date, vendor_id, unit_id, unit_price) 
                    VALUES ('$item_id', '$qty', '$qty', '$bill_no', '$bill_date', '$vendor_id', '$unit_id', '$price')";
            
            if ($conn->query($sql)) {
                $_SESSION['swal_msg'] = "Electrical stock added successfully!";
                $_SESSION['swal_type'] = "success";
...
$vendors = $conn->query("SELECT * FROM vendors WHERE category = 'Electricals' OR id = $current_v_id ORDER BY vendor_name");
...
$page_title = $is_edit ? "Edit Electrical Stock" : "Add Electrical Stock"; 
ob_start();
?>
        ...
        <div class="card-body p-4 p-md-5">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold mb-1">
                        <i class="bi <?= $is_edit ? 'bi-pencil-square text-danger' : 'bi-lightning-charge text-primary' ?> me-2"></i>
                        <?= $is_edit ? "Modify Electrical Stock" : "Electrical Stock Entry" ?>
                    </h4>
                    <p class="text-muted small mb-0">Record and assign electrical components to facilities.</p>
                </div>
                <?php if($is_edit): ?>
            ...
            <form method="POST" id="electricalForm">
                ...
                <div class="form-section-header">
                    <span class="text">Device Identity & Logistics</span>
                    <div class="line"></div>
                </div>
                <div class="row g-3">
                    <div class="col-md-8">
                        <div class="field-wrapper">
                            <label>Electrical Item Type</label>
                            <select name="electrical_item_id" class="form-select searchable-select" required>
                                <option value="" disabled <?= !$is_edit ? 'selected' : '' ?>>Search or select device...</option>
                ...
                <div class="form-section-header">
                    <span class="text">Inventory & Pricing</span>
                    <div class="line"></div>
                </div>
                <div class="row g-3">
                    <div class="col-md-4">
                ...
                <div class="d-flex justify-content-end gap-3 mt-5 pt-3 border-top">
                    <a href="view_electricals.php" class="btn btn-light px-4 text-muted discard-btn" style="border-radius:10px;">
