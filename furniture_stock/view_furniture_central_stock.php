...
if (isset($_POST['delete_id'])) {
    ...
    // Only allow delete if no stock has been dispatched yet (remaining == total)
    $check = $conn->query("SELECT total_qty, remaining_qty FROM furniture_central_stock WHERE id = $del_id")->fetch_assoc();
    
    if ($check && $check['remaining_qty'] == $check['total_qty']) {
        if ($conn->query("DELETE FROM furniture_central_stock WHERE id = $del_id")) {
...
// --- 3. FETCH CENTRAL STOCK ---
$query = "SELECT cs.*, fi.item_name, v.vendor_name 
          FROM furniture_central_stock cs
          LEFT JOIN furniture_items fi ON cs.furniture_item_id = fi.id
          LEFT JOIN vendors v ON cs.vendor_id = v.id
          ORDER BY cs.created_at DESC";
$result = $conn->query($query);

$page_title = "Central Furniture Inventory";
ob_start();
?>
...
<div class="container-fluid p-0 mt-3 content-wrapper-full">
    ...
    <div class="d-flex justify-content-between align-items-end mb-4 px-2">
        <div>
            <h3 class="fw-bold mb-1">Central Furniture Warehouse</h3>
            <p class="text-muted small mb-0">Manage bulk procurement and monitor unallocated stock levels.</p>
        </div>
        <a href="add_furniture_central_stock.php" class="btn btn-primary px-4 py-2" style="border-radius: 10px; font-weight: 600;">
            ...
            <div class="row g-3">
                <div class="col-md-12">
                    <div class="search-box-wrapper d-flex align-items-center">
                        ...
                        <input type="text" id="tableSearch" class="form-control border-0 bg-transparent shadow-none" placeholder="Search by Item, Vendor or Bill Number...">
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <small class="text-muted">ID: #CF-<?= $row['id'] ?></small>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="fw-medium"><?= htmlspecialchars($row['vendor_name']) ?></div>
                        <small class="text-primary fw-bold">Bill: <?= htmlspecialchars($row['bill_no']) ?></small>
                    </td>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg" style="border-radius: 12px;">
                                <li>
                                    <form method="POST" action="add_furniture_central_stock.php">
...
$(document).ready(function() {
    ...
    $('.delete-btn').on('click', function() {
        ...
        if(isDispatched == '1') {
            Swal.fire('Locked', 'Cannot delete stock that has already been dispatched.', 'error');
            return;
        }

        Swal.fire({
            title: 'Are you sure?',
            text: "This will permanently remove this bulk entry from inventory.",
            icon: 'warning',
