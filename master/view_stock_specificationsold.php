...
$result = $conn->query("
    SELECT 
        ...
        sd.serial_number,
        sd.warranty_upto,
        COUNT(sd.id) OVER (PARTITION BY m.id) AS pc_count
    FROM item_models m
    JOIN items_master i ON i.id = m.item_id
    LEFT JOIN stock_details sd ON sd.model_id = m.id
    WHERE m.status='Active'
    ORDER BY i.item_name, m.model_name, sd.serial_number
");
...
<h4 class="mb-3">
<i class="bi bi-pc-display"></i> View Models & PCs
</h4>
...
<td>
<?php
$warranty = $row['warranty_upto'];
$today = date("Y-m-d");
$soonDate = date("Y-m-d", strtotime("+30 days"));

if($warranty && $warranty < $today){
    $warrantyClass = "badge bg-danger";
    $icon = '<i class="bi bi-exclamation-octagon-fill"></i>'; // Expired
}
elseif($warranty && $warranty <= $soonDate){
    $warrantyClass = "badge bg-warning text-dark";
    $icon = '<i class="bi bi-exclamation-triangle-fill"></i>'; // Expiring Soon
}
else{
    $warrantyClass = "badge bg-success";
    $icon = '<i class="bi bi-check-circle-fill"></i>'; // Valid
}
?>
