...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['process_dispatch'])) {
    ...
    // Updated to electrical_central_stock
    $check_res = $conn->query("SELECT * FROM electrical_central_stock WHERE id = $central_id");
    $central_item = $check_res->fetch_assoc();

    if ($central_item && $dispatch_qty > 0 && $dispatch_qty <= $central_item['remaining_qty']) {
        $conn->begin_transaction();
        try {
            // Deduct from Central Electrical Stock
            $new_remaining = $central_item['remaining_qty'] - $dispatch_qty;
            $conn->query("UPDATE electrical_central_stock SET remaining_qty = $new_remaining WHERE id = $central_id");

            // Add to Unit Electrical Stock
            $item_id = $central_item['electrical_item_id'];
            ...
            // Insert into electrical_stock
            $sql_insert = "INSERT INTO electrical_stock (electrical_item_id, total_qty, available_qty, bill_no, bill_date, vendor_id, unit_id, unit_price) 
                           VALUES ('$item_id', '$dispatch_qty', '$dispatch_qty', '$bill_no', '$bill_date', '$vendor_id', '$unit_id', '$price')";
            $conn->query($sql_insert);

            $conn->commit();
            $_SESSION['swal_msg'] = "Electrical items dispatched successfully!";
...
// --- 3. FETCH DATA ---
// Join with electrical_items
$available_stock = $conn->query("SELECT cs.*, ei.item_name FROM electrical_central_stock cs 
                                 JOIN electrical_items ei ON cs.electrical_item_id = ei.id 
...
<style>
    ...
    .form-section-header .text { padding: 0 15px; font-weight: 800; font-size: 0.85rem; color: #ff9f1c; text-transform: uppercase; letter-spacing: 1px; }

    .field-wrapper { background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 10px; padding: 8px 12px; transition: all 0.2s ease; }
    .field-wrapper:focus-within { border-color: #ff9f1c; background: #fff; box-shadow: 0 0 0 3px rgba(255, 159, 28, 0.05); }
    ...
    .stock-counter { background: #ff9f1c; color: #fff; padding: 2px 8px; border-radius: 5px; font-size: 0.75rem; font-weight: 700; }
    .btn-electrical { background: #ff9f1c; border: none; color: white; }
    .btn-electrical:hover { background: #e68a00; color: white; }
</style>
    ...
    <div class="card main-card">
        <div class="card-body p-4 p-md-5">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold mb-1"><i class="bi bi-lightning-charge text-warning me-2"></i>Dispatch Electrical Stock</h4>
                    <p class="text-muted small mb-0">Issue electrical components from Central Inventory to specific Units.</p>
                </div>
            </div>
            ...
            <form method="POST" id="dispatchForm">
                ...
                <div class="form-section-header">
                    <span class="text">Stock Source</span>
                    <div class="line"></div>
                </div>
                <div class="row g-3">
                    <div class="col-md-8">
                        <div class="field-wrapper">
                            <label>Select Electrical Item</label>
                            <select name="central_stock_id" id="central_stock_id" class="form-select searchable-select" required>
                                ...
                                <?php while($s = $available_stock->fetch_assoc()): ?>
                                    <option value="<?= $s['id'] ?>" data-max="<?= $s['remaining_qty'] ?>">
                                        <?= htmlspecialchars($s['item_name']) ?> (Bill: <?= $s['bill_no'] ?> | Avail: <?= $s['remaining_qty'] ?>)
                                    </option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                ...
                <div class="form-section-header">
                    <span class="text">Destination Details</span>
                    <div class="line"></div>
                </div>
                <div class="row g-3">
                    <div class="col-md-6">
                ...
                <div class="d-flex justify-content-end gap-3 mt-5 pt-3 border-top">
                    <a href="view_electrical_central_stock.php" class="btn btn-light px-4 text-muted" style="border-radius:10px;">Cancel</a>
                    <button type="submit" name="process_dispatch" class="btn btn-electrical px-5 py-2 fw-bold" style="border-radius:10px;">
                        <i class="bi bi- lightning me-2"></i>Process Dispatch
                    </button>
                </div>
            </form>
        </div>
...
$(document).ready(function() {
    ...
    $('#central_stock_id').on('change', function() {
        ...
        if(max) {
            $('#max_badge').text('MAX: ' + max).removeClass('d-none');
    ...
    $('#dispatchForm').on('submit', function(e) {
        ...
        if (qty > max) {
            e.preventDefault();
            Swal.fire('Insufficient Stock', 'Only ' + max + ' units available in central stock.', 'warning');
        }
    });
});
</script>
